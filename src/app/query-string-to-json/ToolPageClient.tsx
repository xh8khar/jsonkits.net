...
import { queryStringToJson, jsonToQueryString } from '@/lib/converters'

const example = `name=JSONKits&version=1&tags%5B0%5D=tool&tags%5B1%5D=converter`

export default function ToolPageClient() {
  return (
    <>
      <ToolLayout
        title="Query String to JSON Converter"
        description="Parse query strings back into JSON format instantly."
        inputPlaceholder="Paste your input here..."
        outputPlaceholder="Converted output will appear here..."
        convertLabel="Parse to JSON"
        onConvert={queryStringToJson}
        exampleInput={example}
        inputLanguage="querystring"
        outputLanguage="json"
        bidirectional
      ...
      <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 prose prose-slate dark:prose-invert max-w-none">
        ...
        <ul>
          <li><strong>REST APIs</strong>: Filtering resources with query parameters.</li>
          <li><strong>Search Engines</strong>: Encoding search queries and filters.</li>
          <li><strong>Web Applications</strong>: Client-side routing with URL state.</li>
        </ul>
        <h2>What Is JSON?</h2>
          ...
          <table className="min-w-full border-collapse border border-slate-300 dark:border-slate-600 my-4">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-800">
                ...
                <th className="border border-slate-300 dark:border-slate-600 px-4 py-2 text-left font-semibold">Query Strings</th>
              </tr>
            </thead>
            <tbody>
              <tr>
        ...
        <h2>How to Convert Query Strings to JSON Online</h2>
        <ol>
          <li><strong>Paste Input</strong>: Copy your query strings data and paste it into the input editor.</li>
          <li><strong>Click Convert</strong>: Press the "Parse to JSON" button to transform your data.</li>
        ...
        <p>Yes. The tool accurately parses query strings input and converts it to valid JSON while preserving all data.</p>
        ...
        <p>Converting between JSON and query strings is a common task in modern development workflows. Developers use this conversion for data transformation, format migration, and interoperability between different systems and tools.</p>
        <h2>Related Tools</h2>
        <ul>
          <li><a href="/query-string--to-json">Reverse Converter</a> — Convert query strings back to JSON</li>
