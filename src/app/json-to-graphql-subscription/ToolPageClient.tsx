...
import { jsonToGraphqlSubscription } from '@/lib/converters'

const example = JSON.stringify({ name: 'JSONKits', version: 1, features: ['json-to-graphql-subscription', 'validator', 'converter'], active: true }, null, 2)

export default function ToolPageClient() {
  return (
    <>
      <ToolLayout
        title="JSON to GraphQL Subscription"
        description="Generate GraphQL subscription definitions from JSON data structure templates for real-time APIs."
        inputPlaceholder="Paste your JSON here..."
        outputPlaceholder="JSON to GraphQL Subscription output will appear here..."
        convertLabel="Convert"
        onConvert={jsonToGraphqlSubscription}
        exampleInput={example}
        inputLanguage="json"
        outputLanguage="text"
      />
      <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 prose prose-slate dark:prose-invert max-w-none">
        <h2>What Is GraphQL Subscription?</h2>
        <p>JSON to GraphQL Subscription is a free online tool that converts JSON data into graphql subscription format for use in various programming environments. This tool is 100% client-side and processes all data locally in your browser.</p>
        <h2>How to Use</h2>
        <ol>
