...
import { jsonToQueryString, queryStringToJson } from '@/lib/converters'

const example = `{"name":"JSONKits","version":1,"tags":["tool","converter"]}`

export default function ToolPageClient() {
  return (
    <>
      <ToolLayout
        title="JSON to Query String Converter"
        description="Convert JSON objects to query string format for URLs instantly."
        inputPlaceholder="Paste your input here..."
        outputPlaceholder="Converted output will appear here..."
        convertLabel="Convert to Query"
        onConvert={jsonToQueryString}
        exampleInput={example}
        inputLanguage="json"
        outputLanguage="querystring"
        bidirectional
        onReverse={queryStringToJson}
      />
      <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 prose prose-slate dark:prose-invert max-w-none">
          ...
          <table className="min-w-full border-collapse border border-slate-300 dark:border-slate-600 my-4">
            ...
            <tbody>
              <tr>
                ...
                <td className="border border-slate-300 dark:border-slate-600 px-4 py-2">are the part of a URL after the ? that contains data in key-value format</td>
              </tr>
              <tr>
        ...
        <h2>How to Convert JSON to Query Strings Online</h2>
        <ol>
          <li><strong>Paste JSON</strong>: Copy your JSON and paste it into the input editor.</li>
          <li><strong>Click Convert</strong>: Press the "Convert to Query" button to transform your data.</li>
        ...
        <h2>Related Tools</h2>
        <ul>
          <li><a href="/query-string">Reverse Converter</a> — Convert query strings back to JSON</li>
