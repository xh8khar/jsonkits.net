...
import { jsonToFortran } from '@/lib/converters'

const example = JSON.stringify({ name: 'JSONKits', version: 1, features: ['json-to-fortran', 'validator', 'converter'], active: true }, null, 2)

export default function ToolPageClient() {
  return (
    <>
      <ToolLayout
        title="JSON to Fortran"
        description="Generate Fortran derived type definitions from JSON data samples for scientific computing."
        inputPlaceholder="Paste your JSON here..."
        outputPlaceholder="JSON to Fortran output will appear here..."
        convertLabel="Convert"
        onConvert={jsonToFortran}
        exampleInput={example}
        inputLanguage="json"
        outputLanguage="fortran"
      />
      <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 prose prose-slate dark:prose-invert">
        <h2>What Is Fortran?</h2>
        <p>JSON to Fortran is a free online tool that converts JSON data into fortran format for use in various programming environments. This tool is 100% client-side and processes all data locally in your browser.</p>
        <h2>How to Use</h2>
        <ol>
