...
export default function ToolPageClient() {
  ...
  const handleFetch = useCallback(async () => {
    ...
    try {
      ...
      addToast(friendlyConvertError('API JSON Viewer', (e as Error).message), 'error')
      setOutput('')
    } finally {
      setLoading(false)
    }
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
        ...
        <p className="text-slate-600 dark:text-slate-400 max-w-7xl mb-8">View and explore JSON responses from REST API endpoints. Enter an API URL to fetch and display the formatted response.</p>
        ...
        {output && <div ref={outputRef}><OutputPanel value={output} label="API Response" /></div>}
      </div>
