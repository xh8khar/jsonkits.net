...
import { jsonToSqlDelete } from '@/lib/converters'

const example = JSON.stringify({ name: 'JSONKits', version: 1, features: ['json-to-sql-delete', 'validator', 'converter'], active: true }, null, 2)

export default function ToolPageClient() {
  return (
    <>
      <ToolLayout
        title="JSON to SQL DELETE"
        description="Generate SQL DELETE statements with WHERE clauses from JSON data for record removal."
        inputPlaceholder="Paste your JSON here..."
        outputPlaceholder="JSON to SQL DELETE output will appear here..."
        convertLabel="Convert"
        onConvert={jsonToSqlDelete}
        exampleInput={example}
        inputLanguage="json"
        outputLanguage="sql"
      />
      <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 prose prose-slate dark:prose-invert max-w-none">
        <h2>What Is SQL DELETE?</h2>
        <p>JSON to SQL DELETE is a free online tool that converts JSON data into sql delete format for use in various programming environments. This tool is 100% client-side and processes all data locally in your browser.</p>
        <h2>How to Use</h2>
        <ol>
